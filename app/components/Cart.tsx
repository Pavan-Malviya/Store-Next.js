         ...
         onClick={() => cartStore.toggleCart()} className="fixed w-full h-screen left-0 top-0 bg-black/25">
            ...
            }} className="text-sm font-bold pb-12" >Back to store... </button>    
           {
            ...
            cartStore.cart.map((item) => (
               ...
               <motion.div layout>
                    <p className=" font-bold text-primary-focus">{item.name}</p>
                    <motion.div layout className="flex gap-1 ">
                    <p className="text-sm text-bold">Quantity: {item.quantity}</p>
                    <button onClick={() => {
                    ...
                    <p className="font-bold text-primary">{item.unit_amount && FormatPrice(item.unit_amount)}</p>
                </motion.div>
            </motion.div>
            )) }
            ...
            <motion.div layout>
                ...
                {totalPrice > 0 && (<p className="font-bold text-primary">Total: {FormatPrice (totalPrice)}</p>)}
                
                <button onClick={handleButtonClick} className="btn btn-primary w-full my-4">Checkout</button>
